...
module CyberSecBot
  module Roles
    ...
    def self.valid_reaction?(event, role_message_id, emoji)
      event.message.id == role_message_id && event.emoji.name == emoji
    end

    # Assign the specified role to the user
    def self.assign_role(user, server, role_id)
    ...
    def self.register_add_role(bot, role_id, role_message_id, emoji)
