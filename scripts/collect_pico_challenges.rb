...
module CyberSecBot
  module Scripts
    def self.call_for_challenges(number, csrf_token, session_id, cf_clearance)
    ...
    def self.update_challenge_details(master_file, csrf_token, session_id, cf_clearance)
    ...
    def self.clean_challenge_data(master_file)
      # Read the master challenges file
      master_data = JSON.parse(File.read(master_file))
    ...
    def self.move_master_challenges(master_file, location)
    ...
    def self.count_challenges(master_file)
      # Read the master challenges file
      master_data = JSON.parse(File.read(master_file))
